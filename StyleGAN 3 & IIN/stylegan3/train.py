...
def subprocess_fn(rank, c, temp_dir):
    # initializing wandb
    if rank == 0:
        with wandb.init(project='fbgan', dir='/cluster/scratch/ssadat'):
            _subprocess_fn(rank, c, temp_dir)
    else:
        _subprocess_fn(rank, c, temp_dir)
        

def _subprocess_fn(rank, c, temp_dir):
...
def main(**kwargs):
    ...
    # Augmentation.
    if opts.aug != 'noaug':
        c.augment_kwargs = dnnlib.EasyDict(class_name='training.augment.AugmentPipe', xflip=1, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1)
        if opts.aug == 'ada':
